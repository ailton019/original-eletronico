...
function filtrarMenuPorPermissao(usuario) {
    ...
    links.forEach(link => {
        ...
        if (modulo) {
            const podeVer = temPermissaoUsuario(usuario, modulo, 'ver');
...
function temPermissaoUsuario(usuario, modulo, acao = 'ver') {
...
function temPermissao(modulo, acao = 'ver') {
    const usuario = JSON.parse(sessionStorage.getItem('usuario'));
    return temPermissaoUsuario(usuario, modulo, acao);
}
