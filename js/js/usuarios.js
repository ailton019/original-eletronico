...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function carregarPermissoes(perfil) {
        ...
        container.innerHTML = Object.entries(permissoes).map(([modulo, permissoesModulo]) => {
            const permissoesHtml = Object.entries(permissoesModulo).map(([acao, valor]) => `
                <label>
                    <input type="checkbox" ${valor ? 'checked' : ''} disabled>
    ...
    async function salvarUsuario() {
        ...
        const dados = {
            ...
            cargo: cargo,
            ativo: ativo,
            permissoes: PERMISSOES_POR_PERFIL[perfil] || PERMISSOES_POR_PERFIL.basico
        };
        ...
        try {
            if (id) {
                ...
                if (error) throw error;
                mostrarNotificacao('Usuário criado!', 'success');
            }
